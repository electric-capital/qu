...
def test_normalize_entry_shapes():
    ...
    assert ms.normalize_entry({"slot": 3, "descriptor": "  Smart ($$$) "}) == _entry(
        slot=3, descriptor="Smart ($$$)"
    )
    # Out-of-range / non-int slots are dropped, not clamped
    assert ms.normalize_entry({"slot": 0})["slot"] is None
    assert ms.normalize_entry({"slot": ms.MAX_TOP_LEVEL_SLOTS + 1})["slot"] is None
    assert ms.normalize_entry({"slot": "2"})["slot"] is None
    assert ms.normalize_entry({"slot": True})["slot"] is None
...
def test_read_drops_duplicate_slots_from_later_entries(store):
    store.write_text(json.dumps({
        ...
        "models": {
            ...
            "claude-sonnet-5": {"slot": 1, "descriptor": "Dup"},
            "": {"slot": 2},
        },
    }))
    entries = ms.read_model_selection()
    assert entries["claude-opus-4-8"]["slot"] == 1
    assert entries["claude-sonnet-5"] == _entry(slot=None, descriptor="Dup")
    assert "" not in entries


def test_save_roundtrip_and_unset_entries_dropped(store):
    stored = ms.save_model_selection({
        "claude-opus-4-8": {"slot": 2, "descriptor": "Best"},
        "claude-sonnet-5": {"allow_public": False},
        "gemini-3.8-flash": {},  # fully unset -> not persisted
    })
    assert stored == {
        "claude-opus-4-8": _entry(slot=2, descriptor="Best"),
...
def test_public_model_catalog_carries_selection_fields(store):
    ...
    ms.save_model_selection({"claude-haiku-4.5": {"slot": 5, "descriptor": "Cheap", "allow_public": False}})
    rows = {m["id"]: m for m in public_model_catalog()}
...
def test_admin_put_full_replacement(admin_routes):
    body = admin_routes.ModelSelectionUpdate(models=[
        admin_routes.ModelSelectionEntry(id="claude-haiku-4.5", slot=1, descriptor=" Cheapest ($) "),
        admin_routes.ModelSelectionEntry(id="claude-opus-4-8", allow_public=False),
    ...
    assert ms.read_model_selection() == {
        "claude-haiku-4.5": _entry(slot=1, descriptor="Cheapest ($)"),
