...
Beside the Vertex-served models, the registry carries OpenRouter-served entries (provider `"openrouter"`, backend `"openrouter"`): `deepseek/deepseek-v4-flash-0731` (display name "DeepSeek V4 Flash 0731" -- the date suffix is DeepSeek's snapshot version and is kept user-visible so snapshots stay distinguishable; 1,310,720-token input window) and `qwen/qwen3.8-27b` (display name "Qwen3.8 27B"; 1M-token input window).

Registry keys are the OpenRouter model ids verbatim, with an optional `openrouter_model_id` override mirroring `vertex_model_id`. These models are "configured" when an OpenRouter API key exists in the inference-credential store (`effective_api_key("openrouter")` -- see [Inference Providers](inference-providers.md)), independent of any Vertex project config.
...
- **Adaptive thinking**: Gated per model by the `thinking_effort` key in `MODEL_REGISTRY` (currently `"medium"` on `claude-opus-4-8` and `claude-opus-5-5`). When set, every request carries `thinking: {"type": "adaptive"}` plus `output_config: {"effort": <level>}`; models without the key send no `thinking` param, which disables thinking on Opus 4.7/4.8 but runs adaptive thinking at the model's default effort on Opus 5 (thinking on by default).
  ...
  - Thinking blocks are bound to the request prefix they were produced under (system prompt, tools, earlier messages) and, on Opus 5.5, to the producing model: client-side compaction (`chat/compaction.py`) therefore strips `thinking`/`redacted_thinking` blocks from the kept span when it rebuilds an Anthropic history behind the new summary turn (older accounts would only have them ignored; accounts created on or after 2026-08-31 get a 400 when a prefix-mismatched Opus 5.5 block is replayed).
  - A refusal fallback or model switch away from Opus 5.5 runs without its thinking (dropped server-side, not an error).
  - Opus 5.5 also returns the model's between-tool-call narration as progress-update `thinking` blocks instead of `text` blocks, so at the default `display` no interim text streams to the UI between tool calls on that model.
  - Tests: `tests/test_anthropic_thinking.py`, `tests/test_compaction.py`
- **Refusal fallbacks**: Opus-5-class models run safety classifiers that can decline a request with a normal HTTP 200 whose stream ends with `stop_reason: "refusal"` (benign cybersecurity work occasionally trips them; Opus 5.5 adds `bio` and `reasoning_extraction` categories).
  - Models with a `refusal_fallback_models` entry in `MODEL_REGISTRY` (currently `claude-opus-5` and `claude-opus-5-5`, both -> `["claude-opus-4-8"]`, Anthropic's recommended fallback for cyber-category refusals) get the Anthropic SDK's client-side `BetaRefusalFallbackMiddleware` -- Vertex AI has no server-side `fallbacks` request param -- registered on the client.
