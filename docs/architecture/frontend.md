...
- `defaultModel`: Per-user "last-used" default LLM model for new conversations.
  - Sourced only from a fresh `GET /app/api/me` fetch (no localStorage); when the server value is unset/unknown/uncredentialed, falls back to Opus 4.8 (`DEFAULT_MODEL_ID` in `frontend/src/constants/models.ts`) if its credentials are configured, otherwise to the first credentialed model in catalog order (`resolveFallbackModel`; e.g. Gemini 3 Flash on a Gemini-only local instance -- deprecated models are never picked).
